...
import { getAccessControlConditionForWallets, LitProtocolEncryptionProvider } from "@encryption/internal/lit/lit-encryption-provider";
...
describe('lit encryption tests', () => {
  ...
  let provider1: LitProtocolEncryptionProvider;
  let wallet2: DialectWalletAdapterWrapper;
  let provider2: LitProtocolEncryptionProvider;

  beforeEach(() => {
    wallet1 = new DialectWalletAdapterWrapper(NodeDialectWalletAdapter.create());
    provider1 = new LitProtocolEncryptionProvider(wallet1, LitSignatureStore.createInMemory());
    wallet2 = new DialectWalletAdapterWrapper(NodeDialectWalletAdapter.create());
    provider2 = new LitProtocolEncryptionProvider(wallet2, LitSignatureStore.createInMemory());
  });
  ...
  //   const shuffledConditions = (new Array(10)).fill(0).map(() => getAccessControlConditionForWallets(shuffle(keys))); // shuffle each 10 times and get access control conditions
  ...
  test('encrypt/decrypt single member', async () => {
    ...
    console.log(originalMessage, encryptedString, encryptedSymmetricKey);
    // then
  ...
  //   // when
  //   const encryptedSymmetricKey = await provider1.createEncryptedSymmetricKey(otherMembers);
  //   const {encryptedString } = await provider1.encrypt(originalMessage, otherMembers, encryptedSymmetricKey);
  ...
  //   // when
  //   const encryptedSymmetricKey = await provider1.createEncryptedSymmetricKey(otherMembers1);
  //   const {encryptedString } = await provider1.encrypt(originalMessage, otherMembers1, encryptedSymmetricKey);
