...
// NOTE: This private key is for demonstration purposes only! Do not use private keys in production.
// The Dialect SolanaSdkFactory fully supports traditional wallet adapters, and does not need access to the private key in any way.

export const dappAccountAddress = new PublicKey(
  process.env.DAPP_WALLET_PUBLIC_KEY as string, // set it to your dapp wallet address
).toBase58();
...
const sdk: DialectSdk<Solana> = Dialect.sdk(
  ...
  SolanaSdkFactory.create({
    wallet: NodeDialectSolanaWalletAdapter.create(), // don't forget to set DIALECT_SDK_CREDENTIALS env var to client private key
  }),
);

async function main() {
  console.log(`Client wallet address: ${sdk.wallet.address}`);
  console.log(`Dapp address: ${dappAccountAddress}`);
